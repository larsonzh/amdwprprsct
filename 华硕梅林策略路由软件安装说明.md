...
v3.7.3 及以上版本的压缩格式安装包，按压缩格式类别二选一：

- zip 压缩格式安装包：lz_rule-v[版本号].zip（例如：lz_rule-v4.4.3.zip）

  或

- tar.gz 压缩格式安装包（推荐）：lz_rule-v[版本号].tgz（例如：lz_rule-v4.4.3.tgz）或 lz_rule-v[版本号].tar.gz（例如：lz_rule-v4.4.3.tar.gz）
...
`unzip -o lz_rule-v4.4.3.zip`

然后进入解压缩出的目录 lz_rule-v[版本号] 内按所含安装软件实际文件名称执行文件执行权限设置：
...
`tar -xzvf lz_rule-v4.4.3.tar.gz`

### 3. 软件安装命令

解压缩出的目录 lz_rule-v[版本号] 内按所含安装软件实际文件名称执行：
