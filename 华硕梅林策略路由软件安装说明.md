...
- zip 压缩格式安装包：lz_rule-v[版本号].zip（lz_rule-v4.4.3.zip）

  或

- tar.gz 压缩格式安装包（推荐）：lz_rule-v[版本号].tgz（lz_rule-v4.4.3.tgz）或 lz_rule-v[版本号].tar.gz（lz_rule-v4.4.3.tar.gz）
...
然后进入解压缩出的目录 lz_rule-v[版本号]（lz_rule-v4.4.3）内按所含安装软件实际文件名称执行文件执行权限设置：
...
解压缩出的目录 lz_rule-v[版本号]（lz_rule-v4.4.3）内按所含安装软件实际文件名称执行：
