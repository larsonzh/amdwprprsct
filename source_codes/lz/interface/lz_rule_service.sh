            ...
            case "${list_prio}" in
                                ...
                                print_domain_src_rt_list() {
                                    {
                                        ...
                                        if ! ip rule show | grep -qE "^[[:space:]]*${1}:[[:space:]]*from[[:space:]]*all[[:space:]]*fwmark[[:space:]]*${2}[[:space:]]*lookup"; then
                                            ...
                                            ipset -q list "lz_dn_de_src_addr_${3}" \
                                                | awk -v count="0" '/^([0-9]{1,3}[\.]){3}[0-9]{1,3}([\/][0-9]{1,2})?$/ {print $1; count++;} \
                ...
                24960)
                    print_proxy_remote_node_rt_list() {
                        {
                            ip rule show | sed -n "/^[[:space:]]*${1}:/{
                                s/^[[:space:]]*${1}:[[:space:]]*from[[:space:]]*0[\.]0[\.]0[\.]0[[:space:]]*to[[:space:]]*\(\([0-9]\{1,3\}[\.]\)\{3\}[0-9]\{1,3\}\([\/][0-9]\{1,2\}\)\?\)[[:space:]]*lookup.*$/\1/;
                                /^\([0-9]\{1,3\}[\.]\)\{3\}[0-9]\{1,3\}\([\/][0-9]\{1,2\}\)\?$/!d;
                                p
                            }" \
                            | awk -v count="0" -v pid="[${$}]:" 'BEGIN{system("printf \"%s %s \n\n\" \"$( date +\"%F %T\" )\" "pid)} \
                            ...
                            print_iptv_box_ip_rt_list() {
                                {
                                    ...
                                    eval "$( ip rule show | sed -n "/^[[:space:]]*${1}:/{
                                        s/^[[:space:]]*${1}:[[:space:]]*from[[:space:]]*\(\([0-9]\{1,3\}[\.]\)\{3\}[0-9]\{1,3\}\([\/][0-9]\{1,2\}\)\?\)[[:space:]]*lookup.*$/\1/;
                                        s/^[[:space:]]*${1}:[[:space:]]*from[[:space:]]*all[[:space:]]*to[[:space:]]*\(\([0-9]\{1,3\}[\.]\)\{3\}[0-9]\{1,3\}\([\/][0-9]\{1,2\}\)\?\)[[:space:]]*lookup.*$/\1/;
                                        s/^[[:space:]]*${1}:[[:space:]]*from[[:space:]]*\(\([0-9]\{1,3\}[\.]\)\{3\}[0-9]\{1,3\}\([\/][0-9]\{1,2\}\)\?\)[[:space:]]*to[[:space:]]*\(\([0-9]\{1,3\}[\.]\)\{3\}[0-9]\{1,3\}\([\/][0-9]\{1,2\}\)\?\)[[:space:]]*lookup.*$/\1 \4/;
