...
lz_get_route_info() {
	...
	if [ -n "${local_route_product_model}" ]; then
		echo "$(lzdate)" [$$]: LZ Route Model: "${local_route_product_model}" >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: "   Route Model: ${local_route_product_model}"
	fi

	## 输出显示路由器硬件类型
	[ -z "${route_hardware_type}" ] && route_hardware_type="Unknown"
	echo "$(lzdate)" [$$]: LZ Hardware Type: "${route_hardware_type}" >> "${SYSLOG}"
	echo "$(lzdate)" [$$]: "   Hardware Type: ${route_hardware_type}"

	## 输出显示路由器主机名
	local local_route_hostname="$( uname -n )"
	[ -z "${local_route_hostname}" ] && local_route_hostname="Unknown"
	echo "$(lzdate)" [$$]: LZ Host Name: "${local_route_hostname}" >> "${SYSLOG}"
	...
	echo "$(lzdate)" [$$]: LZ Kernel Name: "${local_route_Kernel_name}" >> "${SYSLOG}"
	...
	echo "$(lzdate)" [$$]: LZ Kernel Release: "${local_route_kernel_release}" >> "${SYSLOG}"
	...
	echo "$(lzdate)" [$$]: LZ Kernel Version: "${local_route_kernel_version}" >> "${SYSLOG}"
	echo "$(lzdate)" [$$]: "   Kernel Version: ${local_route_kernel_version}"

	## 输出显示路由器操作系统名称
	[ -z "${route_os_name}" ] && route_os_name="Unknown"
	echo "$(lzdate)" [$$]: LZ OS Name: "${route_os_name}" >> "${SYSLOG}"
	echo "$(lzdate)" [$$]: "   OS Name: ${route_os_name}"
	...
	if [ "${route_os_name}" = "Merlin-Koolshare" ]; then
		...
		echo "$(lzdate)" [$$]: LZ Firmware Version: "${local_firmware_version}" >> "${SYSLOG}"
	...
	[ -n "${local_firmware_build}" ] && {
		echo "$(lzdate)" [$$]: LZ Firmware Build: "${local_firmware_build}" >> "${SYSLOG}"
	...
	[ -n "$local_bootloader_cfe" ] && {
		echo "$(lzdate)" [$$]: LZ Bootloader \(CFE\): "${local_bootloader_cfe}" >> "${SYSLOG}"
	...
	if [ -n "${local_cpu_frequency}" ] || [ -n "${local_memory_frequency}" ]; then
		echo "$(lzdate)" [$$]: LZ CPU clkfreq: "${local_cpu_frequency}" MHz >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: "   CPU clkfreq: ${local_cpu_frequency} MHz"
		echo "$(lzdate)" [$$]: LZ Mem clkfreq: "${local_memory_frequency}" MHz >> "${SYSLOG}"
	...
	if [ -z "${local_cpu_temperature}" ]; then
		...
		[ -n "${local_cpu_temperature}" ] && {
			echo "$(lzdate)" [$$]: LZ CPU temperature: "${local_cpu_temperature}" degrees C >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   CPU temperature: ${local_cpu_temperature} degrees C"
		}
	else
		echo "$(lzdate)" [$$]: LZ CPU temperature: "${local_cpu_temperature}" >> "${SYSLOG}"
	...
	if [ -z "${local_interface_5g2}" ]; then
		[ -n "${local_interface_2g_temperature}" ] && {
			echo "$(lzdate)" [$$]: LZ 2.4 GHz temperature: "${local_interface_2g_temperature}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   2.4 GHz temperature: ${local_interface_2g_temperature}"
		}
		[ -n "${local_wl_txpwr_2g}" ] && {
			echo "$(lzdate)" [$$]: LZ 2.4 GHz Tx Power: "${local_wl_txpwr_2g}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   2.4 GHz Tx Power: ${local_wl_txpwr_2g}"
		}
		[ -n "${local_interface_5g1_temperature}" ] && {
			echo "$(lzdate)" [$$]: LZ 5 GHz temperature: "${local_interface_5g1_temperature}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   5 GHz temperature: ${local_interface_5g1_temperature}"
		}
		[ -n "${local_wl_txpwr_5g1}" ] && {
			echo "$(lzdate)" [$$]: LZ 5 GHz Tx Power: "${local_wl_txpwr_5g1}" >> "${SYSLOG}"
		...
		[ -n "${local_interface_5g1_temperature}" ] && {
			echo "$(lzdate)" [$$]: LZ 5 GHz-1 temperature: "${local_interface_5g1_temperature}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   5 GHz-1 temperature: ${local_interface_5g1_temperature}"
		}
		[ -n "${local_wl_txpwr_5g1}" ] && {
			echo "$(lzdate)" [$$]: LZ 5 GHz-1 Tx Power: "${local_wl_txpwr_5g1}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   5 GHz-1 Tx Power: ${local_wl_txpwr_5g1}"
		}
		[ -n "${local_interface_5g2_temperature}" ] && {
			echo "$(lzdate)" [$$]: LZ 5 GHz-2 temperature: "${local_interface_5g2_temperature}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   5 GHz-2 temperature: ${local_interface_5g2_temperature}"
		}
		[ -n "${local_wl_txpwr_5g2}" ] && {
			echo "$(lzdate)" [$$]: LZ 5 GHz-2 Tx Power: "${local_wl_txpwr_5g2}" >> "${SYSLOG}"
	...
	if [ -n "${local_nvram_usage}" ]; then
		echo "$(lzdate)" [$$]: LZ NVRAM usage: "${local_nvram_usage}" >> "${SYSLOG}"
	...
	[ -z "${local_route_local_info}" ] && {
		echo "$(lzdate)" [$$]: LZ Route Local Info: Unknown >> "${SYSLOG}"
	...
	{
		echo "$(lzdate)" [$$]: LZ Route Status: "${local_route_local_link_status}"
		echo "$(lzdate)" [$$]: LZ Route Encap: "${local_route_local_encap}"
		echo "$(lzdate)" [$$]: LZ Route HWaddr: "${local_route_local_mac}"
		echo "$(lzdate)" [$$]: LZ Route Local IP Addr: "${route_local_ip}"
		echo "$(lzdate)" [$$]: LZ Route Local Bcast: "${local_route_local_bcast_ip}"
		echo "$(lzdate)" [$$]: LZ Route Local Mask: "${route_local_ip_mask}"
	} >> "${SYSLOG}"

	if ip route | grep -q nexthop; then
		if [ "${usage_mode}" = "0" ]; then
			echo "$(lzdate)" [$$]: LZ Route Usage Mode: Dynamic Policy >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Usage Mode: Dynamic Policy"
		else
			echo "$(lzdate)" [$$]: LZ Route Usage Mode: Static Policy >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Usage Mode: Static Policy"
		fi
		if [ "${policy_mode}" = "0" ]; then
			echo "$(lzdate)" [$$]: LZ Route Policy Mode: Mode 1 >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Policy Mode: Mode 1"
		elif [ "${policy_mode}" = "1" ]; then
			echo "$(lzdate)" [$$]: LZ Route Policy Mode: Mode 2 >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Policy Mode: Mode 2"
		else
			echo "$(lzdate)" [$$]: LZ Route Policy Mode: Mode 3 >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Policy Mode: Mode 3"
		fi
		if [ "${wan_access_port}" = "1" ]; then
			echo "$(lzdate)" [$$]: LZ Route Host Access Port: Secondary WAN >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Host Access Port: Secondary WAN"
		else
			echo "$(lzdate)" [$$]: LZ Route Host Access Port: Primary WAN >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Host Access Port: Primary WAN"
		fi
		if [ "${route_cache}" = "0" ]; then
			echo "$(lzdate)" [$$]: LZ Route Cache: Enable >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Cache: Enable"
		else
			echo "$(lzdate)" [$$]: LZ Route Cache: Disable >> "${SYSLOG}"
		...
		if [ "${clear_route_cache_time_interval}" -gt "0" ] && [ "${clear_route_cache_time_interval}" -le "24" ]; then
			...
			echo "$(lzdate)" [$$]: LZ Route Flush Cache: Every "${clear_route_cache_time_interval} hour${local_interval_suffix_str}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Route Flush Cache: Every ${clear_route_cache_time_interval} hour${local_interval_suffix_str}"
		else
			echo "$(lzdate)" [$$]: LZ Route Flush Cache: System >> "${SYSLOG}"
...
lz_del_iptv_rule() {
	...
	if [ "${local_ip_rule_exist}" -gt "0" ]; then
		if [ "${1}" = "1" ]; then
			echo "$(lzdate)" [$$]: LZ ip_rule_iptv_"${IP_RULE_PRIO_IPTV}" = "${local_ip_rule_exist}" >> "${SYSLOG}"
		fi
...
lz_delete_ip_rule_output_syslog() {
	...
	do
		...
		if [ "${ip_rule_exist}" -gt "0" ]; then
			echo "$(lzdate)" [$$]: LZ ip_rule_prio_"${local_ip_rule_prio_no}" = "${ip_rule_exist}" >> "${SYSLOG}"
	...
	ip route flush cache > /dev/null 2>&1
	[ "${local_statistics_show}" = "0" ] && echo "$(lzdate)" [$$]: LZ No policy rules in use. >> "${SYSLOG}"
}
...
lz_single_ip_rule_output_syslog() {
	...
	ip_rule_exist="$( ip rule show prio "${local_ip_rule_prio_no}" | wc -l )"
	[ "${ip_rule_exist}" -gt "0" ] && {
		echo "$(lzdate)" [$$]: "   ip_rule_iptv_${local_ip_rule_prio_no} = ${ip_rule_exist}"
		echo "$(lzdate)" [$$]: LZ ip_rule_iptv_"${local_ip_rule_prio_no}" = "${ip_rule_exist}" >> "${SYSLOG}"
	}
}
...
lz_ip_rule_output_syslog() {
	...
	do
		...
		[ "${local_ip_rule_exist}" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ ip_rule_prio_"${local_ip_rule_prio_no}" = "${local_ip_rule_exist}" >> "${SYSLOG}"
			local_statistics_show="1"
		}
		let local_ip_rule_prio_no++
	done
	[ "${local_statistics_show}" = "0" ] && [ "${ip_rule_exist}" = "0" ] && {
		...
		echo "$(lzdate)" [$$]: LZ No policy rules in use. >> "${SYSLOG}"
	}
	echo "$(lzdate)" [$$]: ----------------------------------------
}
...
lz_create_firewall_start_command() {
    ...
    if lz_create_event_interface "${BOOTLOADER_NAME}" "${PATH_LZ}" "${PROJECT_FILENAME}"; then
		echo "$(lzdate)" [$$]: "Successfully registered the firewall-start interface."
		echo "$(lzdate)" [$$]: LZ firewall-start interface successfully registered. >> "${SYSLOG}"
    else
		echo "$(lzdate)" [$$]: "The firewall-start interface registration failed."
		echo "$(lzdate)" [$$]: LZ firewall-start interface registration failed. >> "${SYSLOG}"
    fi
	echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Boot Event Interface ------- >> "${SYSLOG}"
}
...
lz_establish_regularly_update_ispip_data_task() {
	...
	if [ -n "${local_regularly_update_ispip_data_info}" ]; then
		...
		[ -n "${local_ruid_day}" ] && {
			...
			echo "$(lzdate)" [$$]: LZ Update ISP Data: "${local_ruid_hour}:${local_ruid_min} Every ${local_ruid_day} day${local_day_suffix_str}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Regularly Update ISPIP ----- >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: ----------------------------------------
...
lz_ss_support() {
	...
	echo "$(lzdate)" [$$]: Closing Fancyss...... >> "${SYSLOG}"
	echo "$(lzdate)" [$$]: ----------------------------------------
	...
	echo "$(lzdate)" [$$]: Fancyss has been successfully shut down. | tee -ai "${SYSLOG}" 2> /dev/null
	echo "$(lzdate)" [$$]: Restarting Fancyss...... | tee -ai "${SYSLOG}" 2> /dev/null
	...
	echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Fancyss running! ----------- >> "${SYSLOG}"
}
...
echo "\$(lzdate)" [\$\$]: >> "${SYSLOG}"
echo "\$(lzdate)" [\$\$]: Running LZ VPN Event Handling Process "${LZ_VERSION}" >> "${SYSLOG}"
...
EOF_OVPN_SCRIPTS_A
    ...
    if lz_create_event_interface "${OPENVPN_EVENT_NAME}" "${PATH_INTERFACE}" "${OPENVPN_EVENT_INTERFACE_NAME}"; then
		echo "$(lzdate)" [$$]: "Successfully registered the openvpn-event interface."
		echo "$(lzdate)" [$$]: LZ openvpn-event interface successfully registered. >> "${SYSLOG}"
    else
		echo "$(lzdate)" [$$]: "The openvpn-event interface registration failed."
		echo "$(lzdate)" [$$]: LZ openvpn-event interface registration failed. >> "${SYSLOG}"
    fi
	echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" OpenVPN Event Interface ---- >> "${SYSLOG}"
}
	...
	if [ -n "${local_route_list}" ]; then
		...
		if [ -n "${local_vpn_item}" ]; then
			...
			do
				...
				echo "$(lzdate)" [$$]: LZ OpenVPN Server-"${local_index}: ${local_vpn_item}" >> "${SYSLOG}"
			done
			if [ "${local_index}" -gt "0" ]; then
				echo "$(lzdate)" [$$]: "   OpenVPN Client Export: ${local_vpn_client_wan_port}"
				echo "$(lzdate)" [$$]: LZ OpenVPN Client Export: "${local_vpn_client_wan_port}" >> "${SYSLOG}"
				echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" OpenVPN Server running! ---- >> "${SYSLOG}"
			fi
		fi
		...
		if [ "$( nvram get "pptpd_enable" )" = "1" ]; then
			...
			echo "$(lzdate)" [$$]: LZ PPTP Client IP Detect Time: "${vpn_client_polling_time}s" >> "${SYSLOG}"
			local_vpn_item="$( nvram get "pptpd_clients" | sed 's/-/~/g' | sed -n 1p )"
			[ -n "${local_vpn_item}" ] && {
				echo "$(lzdate)" [$$]: "   PPTP Client IP Pool: ${local_vpn_item}"
				echo "$(lzdate)" [$$]: LZ PPTP Client IP Pool: "${local_vpn_item}" >> "${SYSLOG}"
			}
			local_index="0"
			...
			do
				...
				echo "$(lzdate)" [$$]: LZ PPTP VPN Client-"${local_index}: ${local_vpn_item}" >> "${SYSLOG}"
			done
			echo "$(lzdate)" [$$]: "   PPTP Client Export: ${local_vpn_client_wan_port}"
			echo "$(lzdate)" [$$]: LZ PPTP Client Export: "${local_vpn_client_wan_port}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" PPTP VPN Server running! --- >> "${SYSLOG}"
		fi
		...
		if [ "$( nvram get "ipsec_server_enable" )" = "1" ]; then
			...
			if [ -n "${local_vpn_item}" ]; then
				...
				echo "$(lzdate)" [$$]: LZ IPSec Server Subnet: "${local_vpn_item}" >> "${SYSLOG}"
				echo "$(lzdate)" [$$]: "   IPSec Client Export: ${local_vpn_client_wan_port}"
				echo "$(lzdate)" [$$]: LZ IPSec Client Export: "${local_vpn_client_wan_port}" >> "${SYSLOG}"
				echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" IPSec VPN Server running! -- >> "${SYSLOG}"
			fi
...
lz_output_ispip_info_to_system_records() {
	## 输出WAN出口接入的ISP运营商信息
	echo "$(lzdate)" [$$]: LZ "Primary WAN   ${2}" >> "${SYSLOG}"
	echo "$(lzdate)" [$$]: ----------------------------------------
	echo "$(lzdate)" [$$]: "   Primary WAN     ${2}"
	[ "${2}" != "Unknown" ] && {
		if [ "${local_wan0_pub_ip}" = "${local_wan0_local_ip}" ]; then
			echo "$(lzdate)" [$$]: "                         ${local_wan0_pub_ip}"
			echo "$(lzdate)" [$$]: "                       ${local_wan0_pub_ip}" >> "${SYSLOG}"
		else
			echo "$(lzdate)" [$$]: "                         ${local_wan0_local_ip}"
			echo "$(lzdate)" [$$]: "                   Pub   ${local_wan0_pub_ip}"
			echo "$(lzdate)" [$$]: "                       ${local_wan0_local_ip}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "                 Pub   ${local_wan0_pub_ip}" >> "${SYSLOG}"
		fi
	}
	echo "$(lzdate)" [$$]: LZ "Secondary WAN ${3}" >> "${SYSLOG}"
	echo "$(lzdate)" [$$]: ----------------------------------------
	echo "$(lzdate)" [$$]: "   Secondary WAN   ${3}"
	[ "${3}" != "Unknown" ] && {
		if [ "${local_wan1_pub_ip}" = "${local_wan1_local_ip}" ]; then
			echo "$(lzdate)" [$$]: "                         ${local_wan1_pub_ip}"
			echo "$(lzdate)" [$$]: "                       ${local_wan1_pub_ip}" >> "${SYSLOG}"
		else
			echo "$(lzdate)" [$$]: "                         ${local_wan1_local_ip}"
			echo "$(lzdate)" [$$]: "                   Pub   ${local_wan1_pub_ip}"
			echo "$(lzdate)" [$$]: "                       ${local_wan1_local_ip}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "                 Pub   ${local_wan1_pub_ip}" >> "${SYSLOG}"
		fi
	}
	echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" WAN ISP -------------------- >> "${SYSLOG}"
	echo "$(lzdate)" [$$]: ----------------------------------------
	...
	[ "${isp_data_0_item_total}" -gt "0" ] && {
		...
		##     Equal Division--均分出口
		##     Load Balancing--系统负载均衡分配出口
		echo "$(lzdate)" [$$]: LZ "Foreign       $( lz_get_ispip_info "${isp_wan_port_0}" )${local_hd}" >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: "   Foreign         $( lz_get_ispip_info "${isp_wan_port_0}" )${local_hd}"
		local_exist="1"
	}
	local local_index="1"
	until [ "${local_index}" -gt "${ISP_TOTAL}" ]
	do
		[ "$( lz_get_isp_data_item_total_variable "${local_index}" )" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ "$local_isp_name$( lz_get_ispip_info "$( lz_get_isp_wan_port "${local_index}" )" )${local_hd}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   $local_isp_name  $( lz_get_ispip_info "$( lz_get_isp_wan_port "${local_index}" )" )${local_hd}"
			local_exist="1"
		}
		let local_index++
	done
	[ "${local_exist}" = "1" ] && {
		echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" ISPIP Policy --------------- >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: ----------------------------------------
	}
	local_exist="0"
	...
	[ "${custom_data_wan_port_1}" -ge "0" ] && [ "${custom_data_wan_port_1}" -le "2" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ "Custom-1      $( lz_get_ispip_info "${custom_data_wan_port_1}" )${local_hd}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Custom-1        $( lz_get_ispip_info "${custom_data_wan_port_1}" )${local_hd}"
			local_exist="1"
		}
	}
	[ "${custom_data_wan_port_2}" -ge "0" ] && [ "${custom_data_wan_port_2}" -le "2" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ "Custom-2      $( lz_get_ispip_info "${custom_data_wan_port_2}" )${local_hd}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Custom-2        $( lz_get_ispip_info "${custom_data_wan_port_2}" )${local_hd}"
			local_exist="1"
		}
	}
	[ "${wan_1_client_src_addr}" = "0" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ "SrcLst-1      Primary WAN${local_hd}" >> "${SYSLOG}"
	...
	[ "${wan_2_client_src_addr}" = "0" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ "SrcLst-2      Secondary WAN${local_hd}" >> "${SYSLOG}"
	...
	[ "${high_wan_1_client_src_addr}" = "0" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ "HighSrcLst-1  Primary WAN${local_hd}" >> "${SYSLOG}"
	...
	[ "${high_wan_2_client_src_addr}" = "0" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			...
			echo "$(lzdate)" [$$]: LZ "HighSrcLst-2  Secondary WAN${local_hd}" >> "${SYSLOG}"
	...
	[ "${wan_1_src_to_dst_addr}" = "0" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			local_hd="${local_primary_wan_hd}"
			echo "$(lzdate)" [$$]: LZ "SrcToDstLst-1 Primary WAN${local_hd}" >> "${SYSLOG}"
	...
	[ "${wan_2_src_to_dst_addr}" = "0" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			local_hd="${local_secondary_wan_hd}"
			echo "$(lzdate)" [$$]: LZ "SrcToDstLst-2 Secondary WAN${local_hd}" >> "${SYSLOG}"
	...
	[ "${high_wan_1_src_to_dst_addr}" = "0" ] && {
		...
		[ "${local_item_num}" -gt "0" ] && {
			local_hd="${local_primary_wan_hd}"
			echo "$(lzdate)" [$$]: LZ "HiSrcToDstLst Primary WAN${local_hd}" >> "${SYSLOG}"
	...
	[ "${usage_mode}" = "0" ] && [ "${local_item_num}" -gt "0" ] && {
		local_hd="${local_load_balancing_hd}"
		echo "$(lzdate)" [$$]: LZ "LocalIPBlcLst Load Balancing${local_hd}" >> "${SYSLOG}"
	...
	[ "${local_item_num}" -gt "0" ] && {
		if [ "${iptv_igmp_switch}" = "0" ]; then
			local_hd="${local_primary_wan_hd}"
			echo "$(lzdate)" [$$]: LZ "IPTVSTBIPLst  Primary WAN${local_hd}" >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   IPTVSTBIPLst    Primary WAN${local_hd}"
			local_exist="1"
		elif [ "${iptv_igmp_switch}" = "1" ]; then
			local_hd="${local_secondary_wan_hd}"
			echo "$(lzdate)" [$$]: LZ "IPTVSTBIPLst  Secondary WAN${local_hd}" >> "${SYSLOG}"
	...
	if [ "${iptv_igmp_switch}" = "0" ] || [ "${iptv_igmp_switch}" = "1" ]; then
		if [ "${iptv_access_mode}" = "2" ]; then
			...
			[ "${local_item_num}" -gt "0" ] && {
				echo "$(lzdate)" [$$]: LZ "IPTVSrvIPLst  Available" >> "${SYSLOG}"
	...
	[ "${local_exist}" = "1" ] && {
		echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Customization Policy ------- >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: ----------------------------------------
	}
}
...
lz_output_dport_policy_info_to_system_records() {
	...
	[ -n "${local_dports}" ] && local_item_exist="1" && {
		echo "$(lzdate)" [$$]: LZ "Primary WAN   TCP:${local_dports}" >> "${SYSLOG}"
	...
	[ -n "${local_dports}" ] && local_item_exist="1" && {
		echo "$(lzdate)" [$$]: LZ "Primary WAN   UDP:${local_dports}" >> "${SYSLOG}"
	...
	[ -n "${local_dports}" ] && local_item_exist="1" && {
		echo "$(lzdate)" [$$]: LZ "Primary WAN   UDPLITE:${local_dports}" >> "${SYSLOG}"
	...
	[ -n "${local_dports}" ] && local_item_exist="1" && {
		echo "$(lzdate)" [$$]: LZ "Primary WAN   SCTP:${local_dports}" >> "${SYSLOG}"
	...
	[ -n "${local_dports}" ] && local_item_exist="1" && {
		echo "$(lzdate)" [$$]: LZ "Secondary WAN TCP:${local_dports}" >> "${SYSLOG}"
	...
	[ -n "${local_dports}" ] && local_item_exist="1" && {
		echo "$(lzdate)" [$$]: LZ "Secondary WAN UDP:${local_dports}" >> "${SYSLOG}"
	...
	[ -n "${local_dports}" ] && local_item_exist="1" && {
		echo "$(lzdate)" [$$]: LZ "Secondary WAN UDPLITE:${local_dports}" >> "${SYSLOG}"
	...
	[ -n "${local_dports}" ] && {
		echo "$(lzdate)" [$$]: LZ "Secondary WAN SCTP:${local_dports}" >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: "   Secondary WAN   SCTP:${local_dports}"
	}
	[ "$local_item_exist" = "1" ] && {
		echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Dst Ports Policy ----------- >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: ----------------------------------------
	}
}
...
lz_deployment_routing_policy() {
	...
	if [ "${local_netfilter_used}" = "0" ]; then
		if [ "${local_auto_traffic}" = "0" ]; then
			echo "$(lzdate)" [$$]: LZ "All in High Speed Direct DT Mode." >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Data Transmission ---------- >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   All in High Speed Direct DT Mode."
			echo "$(lzdate)" [$$]: ----------------------------------------
		else
			echo "$(lzdate)" [$$]: LZ "Using Link Load Balancing Technology." >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Data Transmission ---------- >> "${SYSLOG}"
			echo "$(lzdate)" [$$]: "   Using Link Load Balancing Technology."
			echo "$(lzdate)" [$$]: ----------------------------------------
		fi
	else
		echo "$(lzdate)" [$$]: LZ "Using Netfilter Technology." >> "${SYSLOG}"
		echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" Data Transmission ---------- >> "${SYSLOG}"
	...
	if [ "${udpxy_used}" = "0" ]; then
		...
		[ "${local_wan1_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan1_started}" ]; then
				echo "$(lzdate)" [$$]: UDPXY service in Primary WAN \( "${route_local_ip}:${wan1_udpxy_port}" "${local_udpxy_wan1_dev}" \) has been started.
			else
				echo "$(lzdate)" [$$]: Start UDPXY service in Primary WAN \( "${route_local_ip}:${wan1_udpxy_port}" "${local_udpxy_wan1_dev}" \) failure.
			fi
		}
		[ "${local_wan2_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan2_started}" ]; then
				echo "$(lzdate)" [$$]: UDPXY service in Secondary WAN \( "${route_local_ip}:${wan2_udpxy_port}" "${local_udpxy_wan2_dev}" \) has been started.
			else
				echo "$(lzdate)" [$$]: Start UDPXY service in Secondary WAN \( "${route_local_ip}:${wan2_udpxy_port}" "${local_udpxy_wan2_dev}" \) failure.
			fi
		}
		[ "${iptv_igmp_switch}" = "0" ] && {
			if ip route show table "${LZ_IPTV}" | grep -q "default"; then
		...
		[ "${local_wan1_igmp_start}" = "1" ] && {
			if [ -n "${local_igmp_proxy_started}" ]; then
				echo "$(lzdate)" [$$]: Start IGMP service in Primary WAN \( "${local_udpxy_wan1_dev}" \) success. >> "${SYSLOG}"
			else
				if ! which bcmmcastctl > /dev/null 2>&1; then
					echo "$(lzdate)" [$$]: Start IGMP service in Primary WAN \( "$local_udpxy_wan1_dev" \) failure. >> "${SYSLOG}"
				fi
			fi
		}
		[ "${local_wan2_igmp_start}" = "1" ] && {
			if [ -n "${local_igmp_proxy_started}" ]; then
				echo "$(lzdate)" [$$]: Start IGMP service in Secondary WAN \( "${local_udpxy_wan2_dev}" \) success. >> "${SYSLOG}"
			else
				if ! which bcmmcastctl > /dev/null 2>&1; then
					echo "$(lzdate)" [$$]: Start IGMP service in Secondary WAN \( "${local_udpxy_wan2_dev}" \) failure. >> "${SYSLOG}"
				fi
			fi
		}
		[ "${local_wan1_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan1_started}" ]; then
				echo "$(lzdate)" [$$]: Start UDPXY service in Primary WAN \( "${route_local_ip}:${wan1_udpxy_port}" "${local_udpxy_wan1_dev}" \) success. >> "${SYSLOG}"
			else
				echo "$(lzdate)" [$$]: Start UDPXY service in Primary WAN \( "${route_local_ip}:${wan1_udpxy_port}" "${local_udpxy_wan1_dev}" \) failure. >> "${SYSLOG}"
			fi
		}
		[ "${local_wan2_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan2_started}" ]; then
				echo "$(lzdate)" [$$]: Start UDPXY service in Secondary WAN \( "${route_local_ip}:${wan2_udpxy_port}" "${local_udpxy_wan2_dev}" \) success. >> "${SYSLOG}"
			else
				echo "$(lzdate)" [$$]: Start UDPXY service in Secondary WAN \( "${route_local_ip}:${wan2_udpxy_port}" "${local_udpxy_wan2_dev}" \) failure. >> "${SYSLOG}"
			fi
		}
		[ "${iptv_igmp_switch}" = "0" ] && {
			if ip route show table "${LZ_IPTV}" | grep -q "default"; then
				echo "$(lzdate)" [$$]: IPTV STB can be connected to "${local_udpxy_wan1_dev}" interface for use. >> "${SYSLOG}"
				if [ "${iptv_access_mode}" = "1" ]; then
					echo "$(lzdate)" [$$]: IPTV Access Mode: Direct Connection >> "${SYSLOG}"
				else
					echo "$(lzdate)" [$$]: IPTV Access Mode: Service Address >> "${SYSLOG}"
				fi
			else
				echo "$(lzdate)" [$$]: Connection "${local_udpxy_wan1_dev}" IPTV interface failure !!! >> "${SYSLOG}"
			fi
		}
		[ "${iptv_igmp_switch}" = "1" ] && {
			if ip route show table "${LZ_IPTV}" | grep -q "default"; then
				echo "$(lzdate)" [$$]: IPTV STB can be connected to "${local_udpxy_wan2_dev}" interface for use. >> "${SYSLOG}"
				if [ "${iptv_access_mode}" = "1" ]; then
					echo "$(lzdate)" [$$]: IPTV Access Mode: Direct Connection >> "${SYSLOG}"
				else
					echo "$(lzdate)" [$$]: IPTV Access Mode: Service Address >> "${SYSLOG}"
				...
				echo "$(lzdate)" [$$]: Connection "${local_udpxy_wan2_dev}" IPTV interface failure !!! >> "${SYSLOG}"
			fi
		}
		if [ "${iptv_igmp_switch}" = "0" ] || [ "${iptv_igmp_switch}" = "1" ] || [ "${local_wan1_udpxy_start}" = "1" ] \
			|| [ "${local_wan2_udpxy_start}" = "1" ]; then
			echo "$(lzdate)" [$$]: ----------------------------------------
			echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" IPTV Service --------------- >> "${SYSLOG}"
		fi
	fi
...
! ps | grep "${VPN_CLIENT_DAEMON}" | grep -qv "grep" && {
	...
	echo "\$(lzdate)" [\$\$]: >> "${SYSLOG}"
	echo "\$(lzdate)" [\$\$]: ----------------------------------------------- >> "${SYSLOG}"
	echo "\$(lzdate)" [\$\$]: The VPN client route daemon has been started again. >> "${SYSLOG}"
	echo "\$(lzdate)" [\$\$]: -------- LZ "${LZ_VERSION}" VPN Client Daemon ---------- >> "${SYSLOG}"
	echo "\$(lzdate)" [\$\$]: >> "${SYSLOG}"
}
...
EOF_START_DAEMON_SCRIPT
	...
	if ps | grep "${VPN_CLIENT_DAEMON}" | grep -qv "grep"; then
		echo "$(lzdate)" [$$]: The VPN client route daemon has been started. | tee -ai "${SYSLOG}" 2> /dev/null
		echo "$(lzdate)" [$$]: ----------------------------------------
		echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" VPN Client Daemon ---------- >> "${SYSLOG}"
	elif cru l | grep -q "#${START_DAEMON_TIMEER_ID}#"; then
		echo "$(lzdate)" [$$]: The VPN client route daemon is starting... | tee -ai "${SYSLOG}" 2> /dev/null
		echo "$(lzdate)" [$$]: ----------------------------------------
		echo "$(lzdate)" [$$]: -------- LZ "${LZ_VERSION}" VPN Client Daemon ---------- >> "${SYSLOG}"
	fi
...
lz_start_single_net_iptv_box_services() {
	...
	if [ "${udpxy_used}" = "0" ]; then
		...
		[ "${local_wan1_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan1_started}" ]; then
				echo "$(lzdate)" [$$]: UDPXY service \( "${route_local_ip}:${wan1_udpxy_port}" "${iptv_wan0_ifname}" \) has been started.
			else
				echo "$(lzdate)" [$$]: Start UDPXY service \( "${route_local_ip}:${wan1_udpxy_port}" "${iptv_wan0_ifname}" \) failure.
			fi
		}
		[ "${local_wan2_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan2_started}" ]; then
				echo "$(lzdate)" [$$]: UDPXY service \( "${route_local_ip}:${wan2_udpxy_port}" "${iptv_wan1_ifname}" \) has been started.
			else
				echo "$(lzdate)" [$$]: Start UDPXY service \( "${route_local_ip}:${wan2_udpxy_port}" "${iptv_wan1_ifname}" \) failure.
			fi
		}
		[ "${iptv_igmp_switch}" = "0" ] && {
			if ip route show table "${LZ_IPTV}" | grep -q "default"; then
		...
		[ "${local_wan_igmp_start}" = "1" ] && {
			if [ -n "${local_igmp_proxy_started}" ]; then
				echo "$(lzdate)" [$$]: Start IGMP service \( "${iptv_interface_id}" \) success. >> "${SYSLOG}"
			else
				if ! which bcmmcastctl > /dev/null 2>&1; then
					echo "$(lzdate)" [$$]: Start IGMP service \( "${iptv_interface_id}" \) failure. >> "${SYSLOG}"
				fi
			fi
		}
		[ "${local_wan1_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan1_started}" ]; then
				echo "$(lzdate)" [$$]: Start UDPXY service \( "${route_local_ip}:${wan1_udpxy_port}" "${iptv_wan0_ifname}" \) success. >> "${SYSLOG}"
			else
				echo "$(lzdate)" [$$]: Start UDPXY service \( "${route_local_ip}:${wan1_udpxy_port}" "${iptv_wan0_ifname}" \) failure. >> "${SYSLOG}"
			fi
		}
		[ "${local_wan2_udpxy_start}" = "1" ] && {
			if [ -n "${local_udpxy_wan2_started}" ]; then
				echo "$(lzdate)" [$$]: Start UDPXY service \( "${route_local_ip}:${wan2_udpxy_port}" "${iptv_wan1_ifname}" \) success. >> "${SYSLOG}"
			else
				echo "$(lzdate)" [$$]: Start UDPXY service \( "${route_local_ip}:${wan2_udpxy_port}" "${iptv_wan1_ifname}" \) failure. >> "${SYSLOG}"
			fi
		}
		[ "${iptv_igmp_switch}" = "0" ] && {
			if ip route show table "${LZ_IPTV}" | grep -q "default"; then
				echo "$(lzdate)" [$$]: IPTV STB can be connected to "${iptv_wan0_ifname}" interface for use. >> "${SYSLOG}"
				if [ "${iptv_access_mode}" = "1" ]; then
					echo "$(lzdate)" [$$]: IPTV Access Mode: Direct Connection >> "${SYSLOG}"
				else
					echo "$(lzdate)" [$$]: IPTV Access Mode: Service Address >> "${SYSLOG}"
				...
				echo "$(lzdate)" [$$]: Connection "${iptv_wan0_ifname}" IPTV interface failure !!! >> "${SYSLOG}"
			fi
		}
		[ "${iptv_igmp_switch}" = "1" ] && {
			if  ip route show table "${LZ_IPTV}" | grep -q "default"; then
				echo "$(lzdate)" [$$]: IPTV STB can be connected to "${iptv_wan1_ifname}" interface for use. >> "${SYSLOG}"
				if [ "${iptv_access_mode}" = "1" ]; then
					echo "$(lzdate)" [$$]: IPTV Access Mode: Direct Connection >> "${SYSLOG}"
				else
					echo "$(lzdate)" [$$]: IPTV Access Mode: Service Address >> "${SYSLOG}"
				...
				echo "$(lzdate)" [$$]: Connection "${iptv_wan1_ifname}" IPTV interface failure !!! >> "${SYSLOG}"
			fi
		}
		if [ "${iptv_igmp_switch}" = "0" ] || [ "${iptv_igmp_switch}" = "1" ] || [ "${local_wan1_udpxy_start}" = "1" ] \
			|| [ "${local_wan2_udpxy_start}" = "1" ]; then
