...
软件具有 Web 操作页面，直接嵌入 Asuswrt-Merlin 原生界面，风格保持一致，支持梅林 384.5 及以上的固件版本。页面位于「外部网络(WAN) - 策略路由」，程序启动时自动挂载，全程提供内容详尽的悬浮式帮助，同时可在操作页面上根据最软件新版本的提示信息，直接进行软件的在线更新。如未出现「 策略路由」页面，说明所用固件不支持该功能。
